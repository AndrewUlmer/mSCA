...
def detect_deflect(Z: dict, msca: object, trial_num=0) -> np.ndarray:
    ...
    if hasattr(msca.model.decoder, "model"):
        decoder_weight = np.array(msca.model.decoder.model.weight.data)  # type: ignore
    else:
        decoder_weight = np.array(
            (
                msca.model.decoder.output_layer.weight
                @ msca.model.decoder.hidden_layer.weight
            ).data
        )
